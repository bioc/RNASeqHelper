...
Here we create 9 RNA samples with slighly varying means and SDs, for 10000 genes.
...
"myanalysis/0_genes/smallestGroup9-detected10-keep_genes" file
...
res <- run_deseq(mat, keep_genes, phenotype_data,
         out_dir = "myanalysis/1_matrices_and_deseq")
```

Here tables and plots were written to the "1_matrices_and_deseq" folder:

- `deseq2obj.rds`, an RData object of the DESeq2 object, useful mostly for debugging.
...
- `input_matrix.filt.normalized.tsv`, the above filtered matrix, normalized by DESeq2.
- `input_matrix.filt.normalized.vst_corrected.tsv`, the filtered matrix with a Variance Stabilizing Transform to correct the data.
- `input_matrix.filt.normalized.vst_corrected_PCA.pdf`, a PCA of the normalized and transformed values. 
...
pairwise_hmap_volcano(res$ddsObj,
                    ...
                    kmeans_list = c(2,5,8),
                    out_dirprefix="myanalysis/2_heatmaps")
...
It is entirely possible to run the complete analysis above with a single function:


```{r}
rnaseqhelper(mat, phenotype_data, out_dir="myanalysis2", 
             numer = "green", denom = "blue",
             keep_params = list(min_occur=8, min_detect=20),
             heat_params = list(top_ngenes_tocluster = 2000,
                                top_ngenes_tohighlight = 25,
                                score_thresh = c(0.5, 0.9),
                                genes_of_interest = gois_list,
                                kmeans_list = c(2,5,8)))
